...
本文件只是**命令字典**。范围来源优先级、历史成立与当前状态的区分、以及快照绑定（结论只对 `audit.json` 里那个 `snapshot` 负责）都在 [../SKILL.md](../SKILL.md) 的「工作流」一节，本文件不重复。

「这条是本次变更引入的，还是本来就有的」由你在 `decide --why` 里写清，并用 `note --kind disclosure` 披露；本文件只负责把 base/head、历史实现、可达性和增量影响取成可复核的观察。
...
`blame` 只定位历史线索，**不判断责任**。这里只负责收集 base/head、历史实现、可达性和增量影响的直接观察；「变更引入 vs 现存风险」的结论由主代理在 `decide --why` 里给出，本文件不重复枚举语义。
