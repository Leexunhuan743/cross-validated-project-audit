...
仅在风险地图命中对应 OS、编码、语言、构建模式或第三方库语义时读取本文件。每项都要求目标平台、锁定版本或对应版本权威契约；不能靠记忆或其他平台模拟升级为 `CONFIRMED`。
...
- Linux 上的字符串比较或模拟只能形成候选，不能确认 Windows 文件系统结论。
...
| 证据 | 允许裁决 |
|---|---|
| 目标平台/版本真实公共路径复现 | 可进入 `CONFIRMED`，仍需检查反证和影响 |
| 对应版本权威契约明确且代码路径完整 | 可确认静态契约主张 |
| 其他平台模拟、不同版本试验、模型记忆 | 仅 `CONDITIONAL` 或调查线索 |
| 无法获得目标环境或版本证据 | 披露验证缺口，不推测阻断 |
