...
本文件只是**平台陷阱字典**——告诉你哪些地方会骗人、用什么命令能验出来。它不含任何裁决规则：证据强度与判别探针三纪律见 [../SKILL.md](../SKILL.md) §6；路径与分区纪律见 §3 步骤 3。
...
因此：隔离实验产物严格放在 `.audits/<auditId>/scratch/<R_ID>-<EXECUTOR>/` 分片目录下（绝不落在被审计目标树内，也不放到无从追溯的系统临时目录）；引用路径一律写显式绝对路径，不依赖上一次调用留下的 shell 状态；实验用完即清，收口时清空。主文件 §3 步骤 2 与步骤 3 已把这条列为硬约束，这里只补一句平台侧后果。
...
- 只有对应版本权威契约且代码路径完整 → `ES2`，但若 Finding 依赖真实状态或集成语义，仍要记目标环境验证缺口；
