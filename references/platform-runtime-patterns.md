...
**按需读取**：风险地图命中某个具体 OS、编码、shell、语言版本、构建模式或第三方库语义时读本文件；未命中上述场景时跳过本文件。

本文件只是**平台陷阱字典**——告诉你哪些地方会骗人、用什么命令能验出来。它不含任何裁决规则：证据必须由 `run` 代跑、裁决与严重度归主代理，见 [../SKILL.md](../SKILL.md) 的「三条硬规则」；工作区与路径纪律见「命令面」一节。

有一条贯穿全文：**目标平台的真实行为只能由目标平台证明**。模型记忆、其他平台的模拟、不同版本的试验都不是该平台的直接观察，只能写进 `counter`／`notes`，或者由主代理写进 `decide --why`。这条在主文件里的对应后果是：在别的平台或测试架上跑通的结论，**不能充当 `Critical/High` 所需的按预期复现记录**（`decide` 照常会要求一次真实复现），本文件不再重复。
...
因此：隔离实验产物严格放在审计工作区的分片目录里（`<audit-dir>/scratch/<unit>/`，判别性探针放 `<audit-dir>/probes/<unit>/`——绝不落在被审计目标树内，也不放到无从追溯的系统临时目录）；引用路径一律写显式绝对路径，不依赖上一次调用留下的 shell 状态；实验用完即清，收口时清空。任务书里已把这条列为权限边界，这里只补一句平台侧后果。

## Unicode、字节与规范化边界

- 区分 code point、grapheme cluster、UTF 字节、显示宽度和存储长度——五者可以同时不相等。
...
- defensive-only、被类型系统排除或真实输入不可达的情况，不升级为运行时缺陷。
...
```javascript
// Windows 上跑 .bat：路径必须显式加双引号，否则空格会被截断
execFile('"C:\\Program Files\\tool\\run.bat"', { windowsHide: true, shell: true })
```
...
- 优先调用真实 API 的最小公共路径；在无法执行运行时验证时，引用对应版本的官方文档、源码或发布说明。
- 检查默认值、错误类型、线程安全、编码、持久化、兼容窗口和弃用行为。
- 包名相同、代理共识或过去版本的行为，都不能单独确认当前语义。

## 取证后怎么落回主文件

本文件只负责把平台语义变成可复核的观察。拿到观察后：

- 在目标平台/版本的真实公共路径上可重复复现 → 用 `run` 记一条运行记录（按预期复现的那条就是 `Critical/High` 需要的复现记录）；
- 只有对应版本权威契约且代码路径完整 → 可以进 `run` 的静态阅读槽位，但 confidence 只能 Low，且必须 `--allow-static`；若发现依赖真实状态或集成语义，还要写清目标环境验证缺口；
- 其他平台模拟或不同版本试验 → 写进 `counter`／`notes` 作旁证，不能单独确认目标平台语义；
- 拿不到目标环境 → 用 `note --kind residual` 披露缺口及其影响，不推测 Decision、不推放行结论。
