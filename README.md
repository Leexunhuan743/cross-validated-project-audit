...
安全审计和修复验证是目标画像，不与 PR/项目/作者范围互斥。例如“审这个 PR 的安全修复是否有效”可表示为 `scopeMode=pr` + `objectiveProfile=security,fix-verification`。
...
| “审这个 PR，告诉我能不能合并” | `scopeMode=pr` + 门禁报告 | change-attributable 风险、回归、交付完整性 |
...
| “审完后直接修复本地问题” | 上述任一范围 + `executionMode=audit-and-fix` | 审计完成后按 Finding 分批修复并独立复核 |

## 核心机制

- **契约先于方法**：派发前固定 target、scope、objectives、risk tolerance、evidence 与 deliverable；只有会改变结论的歧义才询问。
...
- **风险先于代理**：先确定风险面与可判定不变量，再选择验证方法，最后才分配 agent；代理数量不是覆盖指标。
- **共享事实，隔离判断**：`audit.md` 统一提供 target/base/head/scope，`project-map.md` 只补充术语、入口、changed files 等可复用 DIRECT 事实；隔离 Hypothesis、Finding、Decision 和预期答案。
- **方法异质性降低相关错误**：最高风险不变量至少两个不同 archetype；两个代理重复相同方法只算冗余复核。
- **反证优先**：每个准备提升为 Finding 的 material Hypothesis 都必须记录 counter-hypothesis、预期安全行为、实际反证搜索和结果。
- **Severity ≠ Confidence**：风险维度先映射 Severity，Confidence 独立表示“有多确定”；单条 Evidence 另用 Strength + Reproducibility 描述质量。
- **变更风险 ≠ 现存风险**：需要变更归因时记录 `INTRODUCED / EXPOSED / REGRESSED / PRE_EXISTING / UNKNOWN` Provenance；归因不适用写 `—`，不把审查中发现的旧 bug 误报成本次变更引入。
- **报告面向决策**：默认先输出 Executive report（Gate / Top risks / Required actions / Residual uncertainty），再给可追溯 Audit appendix。
- **状态持久化与断点恢复**：正常模式把任务契约、范围、账本、覆盖矩阵与 findings 持久化到 `.audits/`；不可写时使用同构会话状态并披露跨会话恢复能力降低。
- **显式停止条件**：required coverage、material Hypothesis、Finding Decision 和 residual risks 闭环后停止；连续两轮无 material delta 时不再无依据扩张。关键 Evidence 客观不可得时可用 `INCOMPLETE` 完成。

## 输出与 Gate

默认报告先给决策，再给追溯：**Executive report → Audit appendix**。Executive report 优先回答“能否合并/发布、最大风险、必须做什么、还有什么没验证”；完整 H/E/F/Decision、coverage、probes 与 commit matrix 只在附录或追溯模式展开。

| Gate | 含义 |
|---|---|
| `READY` | 当前任务契约和 required coverage 已闭环，没有已知阻断或条件项 |
| `READY-WITH-CONDITIONS` | 没有阻断项，但仍有明确非阻断条件、Medium/Low 待处理项或残留风险 |
| `BLOCKED` | 存在对当前门禁目标相关的已确认 Critical/High 风险，且尚未完成处置 |
...
- 默认 `executionMode=audit-only`；只有用户要求实施本地修复时才进入 `audit-and-fix`。提交、推送、PR 操作、部署及生产/外部写入仍需分别授权。
- `Risk tolerance` 只影响门禁策略和明确的风险接受，不改变事实、严重度或证据等级。
...
`SKILL.md` 是唯一调度入口；reference 不互相链式加载。主代理只按当前阶段/风险加载需要的模块。

| 文件 | 类型 | 何时读取 | 内容 |
|---|---|---|---|
| `SKILL.md` | core | 每次使用 | 任务契约、核心原则、模块加载与 §1–§7 主流程 |
| `references/audit-ledger.md` | core-state | 初始化/恢复/写 H/E/F/Decision | 四层状态、project-map、coverage、断点恢复与归档 |
...
| `references/auditor-persona.md` | conditional | 实际派发子代理前 | 调查者模板与独立性纪律 |
...
- **风险驱动调度**：风险面是覆盖主键、验证方法是独立性主键、Agent 只是执行资源；共享事实减少重复劳动，判断隔离保证独立性。
